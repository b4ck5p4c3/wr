...
- ../wr-prof is written by `make MODE=prof`, which mirrors the release flags and adds frame pointers with `-fno-omit-frame-pointer` for a profiler stack walk.
...
- The wr meta table is a key and value store that carries the database API version and the build version. After the migration Main.cpp calls Store::check_api_version, which seeds the api_version key on a fresh database from WR_API_VERSION in src/Common.hpp, and refuses to start with a critical error when the stored version differs, since the schema the older binary reads is no longer the one on disk. WR_API_VERSION is bumped when the schema changes in a way an older server cannot read.
...
- Every response carries a set of security headers, the content-type-options, the frame-options, the referrer-policy, the permissions-policy, and the HSTS. The set is identical on every response, so it is held as the precomputed SECURITY_HEADER_BLOCK constant in src/App and written by the server backend as a byte block, which is passed to the reply through App::emit and through the three auth redirects that bypass emit. The no-store cache-control is set on a JSON reply by reply_json. The backend builds the response head into one buffer and sends it with mg_send, so the mongoose printf path is off the response path.
...
- A site owner edits the name, the url, and the description of an owned site through the `/api/v1/sites/rename` endpoint, and the edit is queued as a `rename` pending action for admin review. The payload carries the name, the url, and the description as a JSON object, and the approval applies them through Store::update_site_details. A repointed url is caught at the review. An admin edits any site directly through the `/api/v1/admin/site` endpoint.
