...
- A user owns sites through the user panel, and an admin approves a site, removes a site, manages the users, and reads the audit trail and the streaming server log tail through the admin panel. The admin panel mutations are recorded to the audit_log through Store::record_audit, the site add, edit, and remove, and the pending approve and reject, and the trail is read through the `/api/v1/admin/audit` endpoint.
