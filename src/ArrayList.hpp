...
#pragma clang loop unroll_count(4)
  ...
  mustuse cold fn clone() const -> ArrayList { return ArrayList{*this}; }
  ...
  fn operator=(ArrayList &&other) noexcept -> ArrayList &
  {
    if (this != &other) {
      destroy_all();
  ...
  cold fn operator=(const ArrayList &other)->ArrayList &
  {
    if (this != &other) {
  ...
  mustuse pure fn is_empty() const noexcept -> bool { return m_length == 0; }
  hot mustuse pure fn operator[](usize i) noexcept -> T & { return m_data[i]; }
  hot mustuse pure fn operator[](usize i) const noexcept -> const T &
  {
    return m_data[i];
  }
  hot mustuse pure fn begin() noexcept -> T * { return m_data; }
  hot mustuse pure fn end() noexcept -> T * { return m_data + m_length; }
  hot mustuse pure fn begin() const noexcept -> const T * { return m_data; }
  hot mustuse pure fn end() const noexcept -> const T *
  {
    return m_data + m_length;
  }

  hot fn push(T value) -> void
  {
  ...
  pure fn allocator() const noexcept -> Allocator { return m_allocator; }

  template <typename... Args>
  hot fn push_managed(Args &&...args) -> void
  {
  ...
  fn remove(usize index) noexcept -> void
  {
  ...
  hot mustuse pure fn back() noexcept -> T &
  {
    ASSERT(m_length > 0, "back() on an empty list");
    return m_data[m_length - 1];
  }
  hot mustuse pure fn back() const noexcept -> const T &
  {
    ASSERT(m_length > 0, "back() on an empty list");
    return m_data[m_length - 1];
  }
  mustuse pure fn front() noexcept -> T &
  {
    ASSERT(m_length > 0, "front() on an empty list");
    return m_data[0];
  }
  mustuse pure fn front() const noexcept -> const T &
  {
    ASSERT(m_length > 0, "front() on an empty list");
    return m_data[0];
  }
...
private:
  ...
  fn swap_elements(usize a, usize b) noexcept -> void
  {
  ...
  template <typename Compare>
  fn insertion_sort(Compare is_less) -> void
  {
  ...
  template <typename Compare>
  fn sift_down(usize root, usize heap_length, Compare is_less) -> void
  {
    loop
    {
      usize largest = root;
  ...
  template <typename Compare>
  fn heap_sort(Compare is_less) -> void
  {
