...
/* An ssrf guard. A probe is allowed only when the url is http or https and its
   host resolves entirely to public addresses, so a probe never reaches a
   loopback, a link-local, or a private address. */
fn is_probe_target_allowed(StringView url) -> bool
{
  char host[256];
...
fn Liveness::sweep() -> void
{
  ...
  for (usize i = 0; i < sites.count(); i++) {
    ...
    if (!is_probe_target_allowed(row.url.view())) {
      if (row.is_reachable)
