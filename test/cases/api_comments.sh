...
overlong_body=$(printf 'x%.0s' {1..501})
overlong_payload='{"body":"'"$overlong_body"'"}'
echo "post-toolong: $(curl -s -b "$UJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d "$overlong_payload" "$api/comments/add")"
...
echo "clamp-neg-offset: $(curl -s "$api/comments?offset=-5&limit=1" | sed "$strip")"
echo "clamp-zero-limit: $(curl -s "$api/comments?offset=0&limit=0" | sed "$strip")"
echo "clamp-huge-limit: $(curl -s -o /dev/null -w '%{http_code}' "$api/comments?offset=0&limit=9999")"
echo "clamp-bad-limit: $(curl -s -o /dev/null -w '%{http_code}' "$api/comments?offset=0&limit=abc")"
echo "list-offset-past: $(curl -s "$api/comments?offset=999&limit=5" | sed "$strip")"
echo "post-malformed: $(curl -s -b "$UJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d '{not json' "$api/comments/add")"
echo "post-array-body: $(curl -s -b "$UJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d '[]' "$api/comments/add")"
echo "approve-missing-id: $(curl -s -b "$AJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d '{}' "$api/admin/comments/approve")"
echo "delete-missing-id: $(curl -s -b "$AJAR" -o /dev/null -w '%{http_code}' -X DELETE -H "$ct" -d '{}' "$api/admin/comments/delete")"
echo "approve-nonexistent: $(curl -s -b "$AJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d '{"id":999}' "$api/admin/comments/approve")"
echo "delete-nonexistent: $(curl -s -b "$AJAR" -o /dev/null -w '%{http_code}' -X DELETE -H "$ct" -d '{"id":999}' "$api/admin/comments/delete")"
echo "delete-wrong-method: $(curl -s -b "$AJAR" -o /dev/null -w '%{http_code}' -X POST -H "$ct" -d '{"id":1}' "$api/admin/comments/delete")"
echo "delete: $(curl -s -b "$AJAR" -X DELETE -H "$ct" -d '{"id":1}' "$api/admin/comments/delete")"
echo "delete-2: $(curl -s -b "$AJAR" -X DELETE -H "$ct" -d '{"id":2}' "$api/admin/comments/delete")"
